	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		//set default
				default:
					...
					{
					   $word          = $db->quote( '%'.htmlspecialchars($word).'%', false );
                    //print_R($word);
					...
					$where_players       = $wheres_players?'(' . implode( ($phrase == 'all' ? ') AND (' : ') OR ('), $wheres_players ) . ')':'';///if $plgParams->get( 'players' ) is OF!! $where_players are empty
		...
		//PLAYERS 
			...
			$query = 'SELECT DISTINCT a.id as playerid, '
			...
			. ' "1" AS browsernav,'
			. ' "0" AS section '        
			. ' FROM #__bl_players as a LEFT JOIN #__bl_players_team AS pt ON a.id = pt.player_id LEFT OUTER JOIN #__bl_teams as t ON t.id = pt.team_id '
			. ($where_players?' WHERE ( '. $where_players .' )':'')
			/*. ' AND a.team_id = t.id '*/
		       /* . ' AND a.published = 1'*/
			...
			$rows_players = $where_players?$db->loadObjectList():array();
//print_r($rows_players);
			...
			foreach($rows_players as $key => $row) 
				...
				$rows_players[$key]->title = $row->lastname." ".$row->firstname." - ".$row->nick." (".$row->teamname.")";
		...
		//PLAYERS EXTRA FIELDS
			...
			. ' AND ex.published=1 '
			. ' AND ex.id=exv.f_id '
			. ' AND exv.uid=a.id '.($where_players_extra?' AND ( '. $where_players_extra .' )':'')
			/*. ' AND a.team_id = t.id '*/
		       /* . ' AND b.access <= '. (int) $user->get( 'aid' )*/
			. ' ORDER BY '. $order_players
			;
			...
			$db->setQuery( $query, 0, $limit );
			$rows_players_extra = $where_players_extra?$db->loadObjectList():array();
			...
			foreach($rows_players_extra as $key => $row) 
				...
				$rows_players_extra[$key]->title = $row->lastname." ".$row->firstname." - ".$row->nick." (".$row->teamname.")";
		...
		//TEAMS
			...
			. ' ORDER BY '. $order_teams
			 ...
			 $db->setQuery( $query, 0, $limit );
			$rows_teams = $where_teams?$db->loadObjectList():array();
		...
		//TEAMS EXTRA FIELDS
			...
			. ' AND ex.published=1 '
			. ' AND ex.id=exv.f_id '
			. ' AND exv.uid=a.id '.($where_teams_extra?' AND ( '. $where_teams_extra .' )':'')
		       /* . ' AND a.published = 1'*/
		       /* . ' AND b.access <= '. (int) $user->get( 'aid' )*/
			. ' ORDER BY '. $order_teams
			 ;
			...
			$db->setQuery( $query, 0, $limit );
			$rows_teams_extra = $where_teams_extra?$db->loadObjectList():array();
		...
		$query = ' select m.id as matchid, '
		       ...
		       . ' m.m_id=md.id '
		       . ' AND md.s_id=s.s_id '
		       . ' AND s.t_id=tour.id '.($where_match?' AND ( '. $where_match .' )':'')
		       . ' ORDER BY '. $order_match
			 ;
			
			$db->setQuery( $query, 0, $limit );
			$rows_match= $where_match?$db->loadObjectList():array();
		...
		$query = ' select m.id as matchid, '
		       ...
		       . ' AND ex.published=1 '
		       . ' AND ex.id=exv.f_id '
		       . ' AND exv.uid=m.id '.($where_match_extra?' AND ( '. $where_match_extra .' )':'')
		      /* . ' AND a.published = 1'*/
		      /* . ' AND b.access <= '. (int) $user->get( 'aid' )*/
		       . ' ORDER BY '. $order_match
			 ;
			...
			$db->setQuery( $query, 0, $limit );
			$rows_match_extra = $where_match_extra?$db->loadObjectList():array();
