...
class JHtmlPaypal
{
    ...
    static function getPaypalForm($options,$cap){
        ...
        if ($length >=1 && $length <= 4)
        {
            ...
            { //sandbox.paypal - test
                $header = "Location: https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=".$options->paypal_email."&item_name=".$options->paypal_org."&amount=".$amount."&no_shipping=0&no_note=1&tax=0&currency_code=".$currency_code."&bn=PP%2dBuyNowBF&charset=UTF%2d8&return=".$options->paypalreturn."&cancel_return=".$options->paypalcancel."&rm=2";
            }
