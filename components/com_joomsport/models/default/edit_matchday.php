...
class edit_matchdayJSModel extends JSPRO_Models
{
	...
	function getMatchesFMD($id){
        ...
        if ($match) {
            foreach($match as $m) {
                $m->venue_name = JHTML::_('select.genericlist',   $is_venue, 'venue_id[]', 'class="inputbox" size="1"', 'id', 'v_name', $m->venue_id);;
            }
        }

        $this->_lists["match"] = $match;

        $this->_lists['venue_name'] = JHTML::_('select.genericlist',   $is_venue, 'venue_id_new[]', 'class=inputbox" size="1"', 'id', 'v_name');
	}
	...
	function getFilterseas(){
		...
		$javascript = "onchange='document.filtrForm.submit();'";	
		$this->_lists['seas_filtr'] = JHTML::_('select.genericlist',   $seasons_as, 'sid', 'class="styled jfsubmit" size="1"'.$javascript, 's_id', 't_name', $this->season_id );	
	...
	function getteamsSeas($sid){
		...
		$this->_lists['teams1'] = JHTML::_('select.genericlist',   $is_team, 'teams1', 'class="inputbox" size="1" id="teams1" style="width:150px;"', 'id', 't_name', 0 );
		$this->_lists['teams2'] = JHTML::_('select.genericlist',   $is_team, 'teams2', 'class="inputbox" size="1" id="teams2" style="width:150px;"', 'id', 't_name', 0 );
