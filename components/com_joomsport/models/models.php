...
class JSPRO_Models{
	...
	function getTournOpt($sid){
		$query = "SELECT s.s_id as id, CONCAT(t.name,' ',s.s_name) as name,t.t_single,s.s_enbl_extra FROM #__bl_tournament as t, #__bl_seasons as s WHERE s.s_id = ".($sid)." AND s.t_id = t.id ORDER BY t.name, s.s_name";
		$this->db->setQuery($query);
...
///////////////////////
		...
		if($showSelect || count($seass)){
			...
			$jqre2 = '<select name="moderseason" id="moderseason" class="styled jfsubmit" size="1" '.$javascript.'>';
